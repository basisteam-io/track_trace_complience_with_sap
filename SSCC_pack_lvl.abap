...
LOOP AT it_attp_failed_sscc INTO ls_attp_failed WHERE incor_level IS NOT INITIAL.
      "INCORRECT SSCC PACKING LEVEL
      CLEAR ls_bapiret.
      ls_bapiret-type       = 'E'.
      ls_bapiret-id         = 'ZEWM_ATTP'.
      ls_bapiret-number     = 'XXX'.   "INCORRECT SSCC PACKING LEVEL
    ...
    ENDLOOP.
    LOOP AT it_attp_failed_sscc INTO ls_attp_failed WHERE sgtin_on_pal IS NOT INITIAL.
      "SGTIN ON SSCC PAL LEVEL
      CLEAR ls_bapiret.
      ls_bapiret-type       = 'E'.
      ls_bapiret-id         = 'ZEWM_ATTP'.
      ls_bapiret-number     = 'XXX'.   "SGTIN ON SSCC PAL LEVEL
      ls_bapiret-message_v1 = ls_attp_failed-sscc.
      SHIFT ls_bapiret-message_v1 LEFT DELETING LEADING '0'.
      APPEND ls_bapiret TO et_bapiret.
    ENDLOOP.